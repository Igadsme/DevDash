...
import { AlertCircle, GitPullRequest, CheckCircle, Circle, SortAsc } from "lucide-react";
...
export default function WhatNeedsMe() {
  ...
  const { data, loading, error, reload } = useApi<{ items: Array<{
    id: string; priority: string; type: string; title: string; repo: string; age: string; context: string; action: string; url?: string | null;
  }> }>("/api/actions");

  if (loading) return <div className="p-6 text-sm" style={{ color: "#6b6b80" }}>Loading priorities…</div>;
